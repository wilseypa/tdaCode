...
# on snert
data <- read.csv("/work-ssd/moitraaa/tomatoDatasets/4_rings.csv", header = FALSE, sep = "")
data <- as.data.frame(data)
data[,ncol(data)] <- NULL
png(filename = "/work-ssd/moitraaa/4ringsData.png", width = 1366, height = 716)
plot(data, main = "Data")
dev.off()
...
for(p in percentList) {
  ...
  png(filename = paste("/work-ssd/moitraaa/output4rings", p, ".png", sep = ""), width = 1366, height = 716)
